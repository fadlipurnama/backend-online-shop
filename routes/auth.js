  ...
  async (req, res) => {
    ...
    try {
      ...
      const imageUrl = req.file
        ? `${process.env.BACKEND_URL}/api/assets/images/userProfile/${req.file.filename}`
        : user.imageUrl; // Jika tidak ada file yang diunggah, gunakan gambar lama

      // Jika ada gambar baru yang diunggah dan pengguna memiliki gambar lama, hapus gambar lama
      if (req.file && user.imageUrl) {
        const oldImageUrl = new URL(user.imageUrl); // Ambil URL gambar lama
        const oldImagePath = path.resolve(
          __dirname,
          "../assets/images/userProfile",
          path.basename(oldImageUrl.pathname)
        );
...
router.put(
  ...
  [
    ...
    body("firstName", "Nama depan tidak boleh kosong")
      .optional()
      .isLength({ min: 1 }),
    body("lastName", "Nama belakang tidak boleh kosong")
      .optional()
      .isLength({ min: 1 }),
    body("phoneNumber", "Nomor telepon tidak valid")
      .optional()
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        return res
          .status(404)
          .json({ success: false, error: "Email tidak terdaftar" });
      }
